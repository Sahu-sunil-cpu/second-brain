...
export default function Document({contentAdded, OnContentAdded} : {contentAdded: boolean, OnContentAdded: () => void}) {
    ...
    const getContent = async () => {
      if (contentAdded == true || Loading == true) {
        const res = await axios.get(`${BACKEND_URL}/v1/secondBrain/content/document`, {
          headers: {
  ...
  
          {
            contents.map(({ type, Link, title, description, _id }: {
              ...
              <span className="py-8 px-4" >
                <Card deleteFunction={() => deleteContent(_id.toString())} Id={_id.toString()}  contentAdded={OnContentAdded}   title={title} type={type} size='lg' variant='primary' description={description} image={Link} />
              </span>
            )
          }

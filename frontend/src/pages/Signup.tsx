...
export default function Signup() {
  ...
  const signup = async () => {
    ...
    console.log(mailRef.current?.value)
     const res = await axios.post(`${BACKEND_URL}/v1/secondBrain/signup`, {
        name: username,
        password,
        mail
    ...
    <div className="h-screen w-screen fixed top-0 left-0">
        <div className="grid grid-cols-3 gap-1">
           ...
           <div className="h-screen  bg-slate-100 col-span-2  mt-40 ml-72">
               ...
               <Input usernameRef={usernameRef} passwordRef={passwordRef} mailRef={mailRef} signup={signup}/>
           </div>
        </div>
    </div>
  </div>
...
function Input({usernameRef, passwordRef, mailRef, signup}: {usernameRef: any, passwordRef: any, mailRef: any, signup: () => void}) {
  ...
  <div className="mb-5">
    <label htmlFor="mail" className="block mb-2 text-sm font-medium text-gray-900 dark:text-black">Your mail</label>
    <input ref={mailRef} type="mail" id="username" className="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-white dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="" required />
  </div>
  <div className="mb-5">
  ...
  <button type="submit" onClick={signup} className="text-black bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Register new account</button>
</form>
  </div>
}